    ...
    <div class="table-area">
      <Table :columns="columns" :data="data" :loading='loading' ref='selection' >
        ...
        <template slot-scope="{ row }" slot="actions">
          <Button type="primary" @click="setting(row)">佣金设置</Button>
        </template>
      </Table>
    </div>
    <Modal title="佣金设置" v-model="showModal" :mask-closable="false">
      <Form  label-position="top" >
        <FormItem label="单次限额(0为无上限)" >
          <Input-number v-model="singleLimit.singleLimit" :min="0"></Input-number>元
        </FormItem>
        <FormItem label="阶梯佣金设置" >
...
import {
  ...
  get_commission_platform_type_list,
  get_commission_detail,
  set_state_commission,
  set_all_enable_commission,
  create_commission_ladder,
  update_commission_ladder,
  delete_commission_ladder,
  create_commission_all,
  update_commission_single_limit
} from "@/service/index";
  ...
  methods: {
    async setState(value, row,index) {
      ...
      this.$Modal.confirm({
        title: `${title}`,
        loading: true,
        content: `<p>是否${title}该佣金设置</p>`,
        onOk: async () => {
          this.btnLoading = true;
          const res = await set_state_commission(row.id, state);
          this.btnLoading = false;
          if (res.status < 400) {
            this.$Message.success(`${title}佣金设置成功`);
            this.$Modal.remove();
            // this.getData();
          } else {
            if (value === 1) {
    ...
    oneKeyEnable(){
      this.$Modal.confirm({
        title: '一键开启',
        loading: true,
        content: '<p>是否一键开启所有佣金设置</p>',
        onOk: async () => {
          const res = await set_all_enable_commission(this.form.id);
          if (res.status < 400) {
    ...
    async setting(row){
      ...
      if(row.id){
        ...
        this.getcommissionDetail(this.profitRuleId)
      }else{
        this.profitRuleId = 0
        this.singleLimit = {
    ...
    async delLadder(row,index){
      if(row.isNew){
        ...
        if(this.profitRuleId){
          this.btnLoading = true
          const res = await delete_commission_ladder(row.id);
          this.btnLoading = false
          if (res.status < 400) {
            this.$Message.success('删除成功');
            this.ladder.splice(index,1);
    ...
    async saveLadder(row,index){
      ...
      if(this.profitRuleId){
        ...
        if(row.id){
          const res = await update_commission_ladder(row.id,row)
          if(res.status<400){
            this.$Message.success('保存一条阶梯佣金成功');
          }
        }else{
          const res = await create_commission_ladder(row)
          if(res.status<400){
            this.$Message.success('保存一条阶梯佣金成功');
          }
        }
    ...
    async ok(){
      ...
      this.btnLoading=true
      if(this.profitRuleId){
        const res = await update_commission_single_limit(this.profitRuleId,this.singleLimit)
        if(res.status<400){
          this.$Message.success('设置佣金规则成功');
        }
      }else{
        ...
        const param = {
          ...this.singleLimit,
          commissionDetails:this.ladder
        }
        const res = await create_commission_all(param)
        if(res.status<400){
          this.$Message.success('设置佣金规则成功');
        }
      }
    ...
    async getcommissionDetail(profitRuleId){
      this.loading = true
      const res = await get_commission_detail(profitRuleId)
      this.loading = false
      if(res.status<400){
        this.singleLimit = {
          singleLimit : res.data.singleLimit
        }
        this.ladder = res.data.commissionDelails
        this.ladder.forEach(v=>{
    ...
    async getData() {
      ...
      const rescommission = await get_commission_platform_type_list(this.form.id,this.query);
      this.loading = false;
      if (resGame.status < 400 && rescommission.status<400) {
        this.data = resGame.data;
        this.data.forEach(v => {
          v.id = 0
          rescommission.data.forEach(r => {
